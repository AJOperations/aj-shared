...
def test_feedback_rejects_oversize_screenshot_before_hq(monkeypatch):
    client, fake = make_client()
    monkeypatch.setattr(
        "aj_shared.fastapi_integration._FEEDBACK_MAX_BYTES",
        4,
    )
    ...
    response = client.post(
        "/api/feedback",
        data={"note": "hello"},
        files={"screenshot": ("screen.png", b"12345", "image/png")},
        headers={"X-Requested-With": "XMLHttpRequest"},
    )

    assert response.status_code == 413
    assert response.json() == {"error": "Screenshot must be 5 MB or smaller."}
    assert fake.calls == []
